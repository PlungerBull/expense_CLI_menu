...
**The quick-add bar shipped whole on 2026-08-25**, all four phases: the pure grammar ([expense/quickadd/](expense/quickadd/)), `expense log "<line>"` on the flat CLI, the TUI LOG bar ([expense/tui/screens/log_bar.py](expense/tui/screens/log_bar.py)), and the save — one `POST /transactions/batch` for the complete rows, one `POST /inbox` per draft, through the shared [expense/batch_write.py](expense/batch_write.py) extracted from the importer. `+` opens it on Home, Transactions and Inbox; the bar-cycle form ([quick_log.py](expense/tui/screens/quick_log.py)) kept its **edit** door and lost its create one. Four `decisions.md` entries carry the why.

**Baseline: 1108 unit tests green (2026-08-25), contract suite 12/12** against the disposable engine on `:8001` (last run 2026-08-25). Nothing has regressed and nothing blocks daily use; the one genuine defect on the queue is a pair of TUI form keys macOS eats before the terminal sees them.

**Open work — one home, [docs/todo.md](docs/todo.md)**, grouped there by kind: one genuine bug (the edit form's `^↑/^↓` field keys are eaten by macOS Mission Control — the replacement is drawn but unpicked), one missing capability (the TUI cannot create an inbox draft), four ergonomics items (shell completions, human-name resolution for reference flags, a move-between-accounts convenience, and a conditional CSV importer), one cosmetic rename, and one **engine-side ask** — `POST /transactions/batch` does not return the failing item's index its spec promises, which is why [expense/batch_write.py](expense/batch_write.py) exists. None of it is parity work and none of it blocks daily use. Reference for what exists: [docs/cli-spec.md](docs/cli-spec.md) (commands) · [docs/cli-runtime.md](docs/cli-runtime.md) (runtime behaviour) · [docs/tui.md](docs/tui.md) (the TUI) · [docs/decisions.md](docs/decisions.md) (why).
