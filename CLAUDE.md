...
All TUI color goes through theme tokens (`resolve_palette`), never hard-coded hex/ANSI in widgets. A guard test enforces this; see [docs/tui-plan.md](docs/tui-plan.md). Since 2026-08-19 the tokens themselves are **ANSI slots**, not hexes: the terminal supplies every colour, which is what makes the app readable on light and dark terminals without detecting anything. A hex in [theme.py](expense/tui/theme.py) would pin the app back to one terminal — guarded by `test_theme_is_ansi_slots_not_hexes`.
