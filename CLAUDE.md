...
- **What stays CLI-specific:** [expense/dates.py](expense/dates.py) (CLI-only date forgiveness), [expense/import_/](expense/import_/) (the `.xlsx` reader), `~/.expense-config` (filesystem token storage). *(`expense/_editor.py` — the `$EDITOR` flow — was deleted 2026-08-16 with `reconcile reorder`, its only consumer; see [docs/backlog.md](docs/backlog.md) Phase 3.)* Mobile and web will solve these problems differently and that's fine — the engine doesn't see the difference.
...
Flat CLI is **complete** (Steps 0–9, plus the `.xlsx` bulk importer `expense import` and `rates list`); every engine endpoint with a CLI surface is wrapped and regression-armored. The questionary `expense menu` shipped at Step 9.5 and was **deleted at Step 10.X (2026-07-02)** — flat commands stay the canonical contract-validator surface; the Textual TUI `expense world` (Step 10) is the interactive surface on the same fetch/write layer. TUI read views and write flows are shipped (Phase 2 closed 2026-07-08 with the Monthly-report screen — every home-menu entry is wired); the 2026-07-02 quality review was fully worked off. **Recovery from the 2026-08 engine rework is complete and verified** (2026-08-16): backlog Phases 1–5 closed, and Phase 5's gate — the contract suite against a real engine, the first such check since the rework — passed. **Phase 6, the additive engine capability the clients hadn't surfaced (inbox hashtags, the People API), closed the same day**, so nothing the rework touched is outstanding. **Phase 7 (doc alignment) closed 2026-08-16; Phase 8 (pre-existing polish) is the only open phase and is being worked in passes** — the deleted-rows column landed 2026-08-16, the discoverability pair (`?` help overlay shipped, command palette **removed**) landed 2026-08-17, the `pilot.pause` sweep landed 2026-08-18 (every TUI pilot test now waits on a condition, not the clock — guarded by [tests/unit/test_suite_hygiene.py](tests/unit/test_suite_hygiene.py)), and the **light-theme item closed 2026-08-19 by deleting its premise** — the TUI now takes every colour from the terminal's own ANSI slots, so it reads on light and dark alike with no detection, no second palette and no `NO_COLOR` mode ([docs/decisions.md](docs/decisions.md) "The terminal supplies the palette"). Baseline: 924 unit tests green, contract suite 12/12 against the disposable engine on `:8001`. Nothing is live-broken. **Phase 8 closed 2026-08-20** — the last item, tui-plan §9's minimum terminal size, was closed with no code: the app degrades and says nothing below 80×24, and that is accepted (80×24 documented as correct, 60×20 as lossy-but-workable; nothing enforces either; [docs/decisions.md](docs/decisions.md) "A terminal too small is the user's to fix"). Open work, none of it urgent: post-Step-9 ergonomics (quick-add parser, shell completions, human-name resolution for reference flags), unblocked now that the gate has passed. Queue: [docs/backlog.md](docs/backlog.md); per-entry engine-change detail: [docs/client-breaking-changes.md](docs/client-breaking-changes.md) — read the entry before touching a surface it names. Live detail: [docs/roadmap.md](docs/roadmap.md) (step table) · [docs/tui-plan.md](docs/tui-plan.md) (TUI phases) · [docs/backlog.md](docs/backlog.md) (active backlog).
