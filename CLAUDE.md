...
**Baseline: 1052 unit tests green (2026-08-25), contract suite 12/12** against the disposable engine on `:8001` (last run 2026-08-20). Nothing is broken and nothing is urgent.

**Open work** — all ergonomics, none of it parity: the quick-add bar (phase 1, the grammar in [expense/quickadd/](expense/quickadd/), and phase 2, `expense log "<line>"` on the flat CLI, both shipped 2026-08-25; the TUI screen is phases 3–4), shell completions, human-name resolution for reference flags, and four smaller items (two of them opened 2026-08-20 — the TUI cannot create an inbox draft, and the edit form's `^↑/^↓` field keys are eaten by macOS Mission Control). One home: [docs/todo.md](docs/todo.md). Reference for what exists: [docs/cli-spec.md](docs/cli-spec.md) (commands) · [docs/tui.md](docs/tui.md) (the TUI) · [docs/decisions.md](docs/decisions.md) (why).
