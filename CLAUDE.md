...
**The quick-add bar shipped whole on 2026-08-25**, all four phases: the pure grammar ([expense/quickadd/](expense/quickadd/)), `expense log "<line>"` on the flat CLI, the TUI LOG bar ([expense/tui/screens/log_bar.py](expense/tui/screens/log_bar.py)), and the save — one `POST /transactions/batch` for the complete rows, one `POST /inbox` per draft, through the shared [expense/batch_write.py](expense/batch_write.py) extracted from the importer. `+` opens it on Home, Transactions and Inbox; the bar-cycle form ([quick_log.py](expense/tui/screens/quick_log.py)) kept its **edit** door and lost its create one. Four `decisions.md` entries carry the why. **On 2026-08-29 the bar grew an account peek**: name an account in the line and that account's ledger fills the room under the staged list — elastic, read-only, and it stays on the last account the bar named (mockup `expense-world-log-account-peek.html`). The same day the bar also gained a **permanent legend row** (the whole grammar, `//note` included), a **bare `$`/`@`/`#` that opens the full list**, **accent-blind name matching** shared by all three pickers ([quickadd/names.py](expense/quickadd/names.py)), and **day+month dates** — `28Aug`, `28/ago/2025`, `1set` — resolving to the current year.

**The two Reports entries became one the same day: `Overview`** ([overview.py](expense/tui/screens/overview.py), `reports.py` renamed) — an Accounts | People band, at most 5 drawn rows per panel, above the sliding four-month grid, which now carries `inflow` / `outflow` / `net` from the one shared `build_range_grid`, so the flat `reports monthly --from/--to` table gained the same two rows. `outstanding.py` was **deleted**: its category tree was already this grid's newest column. The cap is a display cap, never a filter; the band always reads today's balances (no balances-as-of-date endpoint exists) and says so in the title. Mockup `expense-world-reports-merge.html` §12; the why, with nine rejected alternatives, in [docs/decisions.md](docs/decisions.md).

**Baseline: 1149 unit tests green (2026-08-29, CI green on 3.11/3.12/3.13), contract suite 12/12** against the disposable engine on `:8001` (last run 2026-08-25). Nothing has regressed and nothing blocks daily use; the one genuine defect on the queue is a pair of TUI form keys macOS eats before the terminal sees them.

**Open work — one home, [docs/todo.md](docs/todo.md)**, grouped there by kind: one genuine bug (the edit form's `^↑/^↓` field keys are eaten by macOS Mission Control — the replacement is drawn but unpicked), one missing capability (the TUI cannot create an inbox draft), four ergonomics items (shell completions, human-name resolution for reference flags, a move-between-accounts convenience, and a conditional CSV importer), one cosmetic rename, and two **engine-side asks** — `POST /transactions/batch` does not return the failing item's index its spec promises (which is why [expense/batch_write.py](expense/batch_write.py) exists), and there is no way to ask which hashtags a category has been used with. None of it is parity work and none of it blocks daily use. Reference for what exists: [docs/cli-spec.md](docs/cli-spec.md) (commands) · [docs/cli-runtime.md](docs/cli-runtime.md) (runtime behaviour) · [docs/tui.md](docs/tui.md) (the TUI) · [docs/decisions.md](docs/decisions.md) (why).
