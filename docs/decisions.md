...
This **reverses the label half of the 2026-08-16 hashtag sketch's pick E**, which chose `Tags` for the inbox. The *position* that pick chose — last on both lists, before the status marker in the TUI — is untouched; only the word changed. Recorded because a future reader will otherwise find pick E and think the code drifted.
