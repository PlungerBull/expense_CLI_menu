...
2. `expense inbox list` supports `--ready` (excludes items missing required fields) and `--include-deleted`. *(`--include-deleted` **removed 2026-08-16**, backlog Phase 8 — dismissal is final since the engine dropped the inbox restore route on 2026-08-14, so the flag could only list drafts nothing can act on. See [decisions.md](decisions.md) "The inbox has no deleted view".)*
...
> ⚠️ **Partially reworked 2026-08-16 (backlog Phase 4).** The engine's 2026-08-05
> read-time-currency change ([client-breaking-changes.md](client-breaking-changes.md))
> deleted the native cross-account aggregates and the archived category/hashtag
> panels, and made every remaining home aggregate nullable with an
> `unconverted_count`. So item 1's "native + home" is now home-only (one `Home`
> column), item 2 is the accounts panel alone, and a figure the engine could not
> price renders as `3 unrated` — never `0.00`. Categories with nothing spent are
> not drawn. Sketch: [mockups/expense-world-phase4-sketch.html](mockups/expense-world-phase4-sketch.html).
...
1. `expense dashboard` → `GET /v1/dashboard`. Current-month-only (the engine endpoint is current-month-only — there is no `?month=` parameter). Renders:
   - Bank accounts (native balance; the home equivalent is on the account list)
   - People accounts
   - Categories with expandable hashtag-combination breakdown
   - Totals (inflow / outflow / net), home currency
2. `--include-archived` → adds the `archived_accounts` panel.
...
**Verify:** `dashboard` matches the web view semantically. `dashboard --include-archived` surfaces the archived-accounts panel. `reports monthly --date 2026-03` returns a single month. `reports monthly --from 2025-11 --to 2026-04` returns 6 months.
...
- **Archive/delete distinction respected.** `accounts list` accepts `--include-archived` + `--include-deleted`; `categories/hashtags/transactions/reconcile list` accept `--include-deleted` only. *(As shipped, categories and hashtags took `--include-archived` too; they stopped archiving 2026-08-06, leaving accounts — and people, which are accounts — the only archivable resource. `inbox list` also took `--include-deleted` as shipped; it was **removed 2026-08-16**, backlog Phase 8 — it is the one soft-deleting resource with no restore route, so the flag listed rows nothing could act on.)*
...
**Remaining:** the merged open-work queue in [backlog.md](backlog.md). The 2026-08 engine-rework recovery **and** the additive features that followed it are done — Phases 1–6 all closed 2026-08-16 — leaving the polish tail (designer theme tokens; light theme + `NO_COLOR` paths; async edge cases + spinners; remaining Textual pilot tests). The `?` help overlay shipped 2026-08-17 and the command palette was **removed** in the same pass — see [decisions.md](decisions.md). Exit criteria: shippable, feature-complete vs the flat command surface.
...
5. **"Move between accounts" convenience** — one command that writes the two ordinary `POST /transactions` calls a transfer now consists of (out one account, in the other). Re-parked here from backlog Phase 8 on 2026-08-16 by user decision: kept as a future ergonomic, explicitly **not** parity work. The engine has had no transfer concept since 2026-08-10 ([client-breaking-changes.md](client-breaking-changes.md)), so this is purely client-side sugar over two writes — which is also the argument against it, since the CLI's job is to exercise the engine's surface, not to invent one. **[UI]** → needs its own mockup (naming, flags, what the two legs' titles and categories default to) before any code. See [decisions.md](decisions.md) "Transfers are two ordinary transactions".
6. **Human-name resolution for reference flags** — accept either UUIDs or human-readable names on every command that takes an `--account-id` / `--category-id` / `--hashtag-id` / `--reconciliation-id`. CLI-side lookup via the existing list endpoints with disambiguation rules (case-insensitive exact match; reject ambiguous matches with a "be more specific" error pointing at `expense <resource> list`). UUIDs continue to pass through unchanged. Significant feature; deserves its own plan-mode session.
