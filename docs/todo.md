...
feature-complete against the engine; 1108 unit tests green (2026-08-25), contract
...
1. **Shell completions** — zsh, bash, fish. `expense <TAB>` shows commands,
   `expense auth <TAB>` shows subcommands. Lowest-effort discoverability win for the
   flat path.

2. **Human-name resolution for reference flags.** Accept either a UUID or a
...
3. **"Move between accounts" convenience** — one command writing the two ordinary
...
4. **`expense import csv`** — CSV variant of the shipped `.xlsx` importer. Only if a
   real migration needs it; the xlsx path already covered the original one.

5. **The TUI cannot create an inbox draft.** Found 2026-08-20 while binding `+`. The
...
6. **Field navigation in the edit form is broken on macOS.** `⌃↑` is Mission Control
