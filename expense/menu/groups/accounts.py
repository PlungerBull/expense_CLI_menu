...
def _validate_optional_int(raw: str) -> bool | str:
    value = raw.strip()
    if value == "":
        return True
    try:
...
def _prompt_optional_int(label: str) -> tuple[bool, int | None]:
    raw = questionary.text(f"{label} (skip)", validate=_validate_optional_int).ask()
    if raw is None:
        return False, None
    raw = raw.strip()
    if raw == "":
...
def run_create(ctx: typer.Context) -> None:
    ...
    ok, sort_order = _prompt_optional_int("Sort order")
    if not ok:
        return
...
def run_update(ctx: typer.Context) -> None:
    ...
    yn = common.prompt_yes_no(_format_field_label("name", item.get("name")))
    if yn is None:
        return
    if yn:
    ...
    yn = common.prompt_yes_no(_format_field_label("color", item.get("color")))
    if yn is None:
        return
    if yn:
    ...
    yn = common.prompt_yes_no(_format_field_label("sort order", item.get("sort_order")))
    if yn is None:
        return
    if yn:
        ok, new_sort = _prompt_optional_int("New sort order")
        if not ok:
            return
